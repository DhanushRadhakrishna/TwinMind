package com.habits.twinmind.WAVRecorder
...
class WAVRecorder(path : String?) {
    ...
    var tempRawFile: String = "TwinMind.raw"
    var tempWavFile: String = "TwinMind.wav"
    val bpp: Int = 16 //number of bits collected in each sample
    var sampleRate: Int = 44100 //frequency of audio samples collected in Hz
    ...
    private fun createWavFile(tempPath: String, wavPath: String) {
        try {
            ...
            val totalAudioLen = fileInputStream.getChannel().size()
    ...
    fun startRecording() {
        try {
            ...
            val status = recorder!!.getState()
            if (status == 1) {
                recorder!!.startRecording()
                isRecording = true
            }
            recordingThread = Thread(Runnable { this.writeRawData() })
